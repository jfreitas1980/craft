...
app.controller('produtosController', function($scope, $q, buscaWS, callWS, $http, $q, $filter, toaster, $timeout) {
    ...
    $scope.fSaveCloneProp = function(oClone) {
        ...
        buscaWS.get('/WVDF_WS/ws_hcgs3004.wso/f_clone_prop/JSON', 'aProp=' + oClone.proposta + "&aCliente=" + oClone.cliente_id + "&aUsuarioSessao=" + getVariavelURL('aUsuarioSessao')).then(function(data) {
            // debugger;    
            var url = "PCGS3004_01.asp?aUsuarioSessao=" + getVariavelURL('aUsuarioSessao') + "&idCliente=" + oClone.cliente_id + "&idProposta=" + data + "&idClone=1";
            window.top.jaddTab("Proposta #" + data, url);
    ...
    $scope.btnVisualizarProposta = function(oProposta) {
        // debugger;
        var prev = oProposta.previusStat;
