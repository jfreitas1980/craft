var app = angular.module('pcgs21105', ['ui.bootstrap', 'wsDominio', 'diretivas']);

app.controller('pcgs21105Ctrl', ['$scope', 'callWS', '$timeout', function($scope, callWS, $timeout) {
    ...
    var init = function() {}
     ...
     $scope.pessoasChange = function(){
          $scope.dualListArray = [];
        getDualList(1, 0, '');
        getDualList(1, 1, '');
    };

    $scope.acPessoas = function(query) {
        var params = {
            'aUsuarioSessao': aUsuarioSessao,
            'sInicial': query,
        };
        return callWS.get('/WVDF_WS/ws_csag320.wso/fautocomplePessoas/JSON', params).then(function(response) {
    ...
    var getDualList = function(currentPage, aOption, aPessoa) {
        ...
        callWS.get('/WVDF_WS/ws_ccgs211.wso/f_PesquisaCCGS211/JSON', params).then(function(response) {
    ...
    var atualizaWS = function(value) {
        var params = {
            'aUsuarioSessao': aUsuarioSessao,
            'aCSAG320_ID': $scope.pessoa.id,
            'aCCGS210_ID': value.ID,
            'bSel': value.SEL
        };
        //  console.log(params);
        callWS.get('/WVDF_WS/ws_ccgs211.wso/f_AtualizaCCGS211/JSON', params).then(function(response) {});
    }
