...
assignacaoComercial.filter('to_trusted', ['$sce', function($sce){
...
$scope.buscaGrupoEconomico = function(value)
{
  ...
  $scope.clearTableSimulation = function()
  {
    $scope.oConsultEcoportV2 = [
      {"lstArquivos":{}}
    ];
  ...
  $scope.callSimulationFile = function()
  {
    $scope.clearTableSimulation();
    $scope.loadScreen = true;

    var data = 
    {
      "CNPJImportador":$scope.busca
    };

    if($scope.busca == undefined || $scope.busca == "")
    {
      $scope.loadScreen = false;
      return parent.parent.alertify.error("Informe o CNPJ");
    }

    $scope.busca = $scope.busca.replace(/[\/.-]/g, "").trim();
    var url;

    url = $scope.urlAPI + "EDI/Ecoporto/ImportacaoCliente_v2/"+ $scope.busca;    
    //url = "http://localhost:21651/EDI/Ecoporto/ImportacaoCliente_v2/"+ $scope.busca;
        
    $http({
        method : "GET",
        url : url
    }).then(function mySuccess(response) {
                      console.log("consultav2", response);
                      $scope.oConsultEcoportV2 = response.data; 
                      $scope.responseEcoporto = $scope.oConsultEcoportV2.resposta;
                      $scope.loadScreen = false;
    }, function myError(response) { 
            $scope.loadScreen = false;
            $scope.disableEditar = true; 
  ...
  $scope.downloadSimulationFile = function(_oArchive)
  {
    var url;
    url = $scope.urlAPI + 'EDI/Ecoporto/GetTableEcoporto/'+ _oArchive.id_TabelaEcoporto;
    //url =  'http://localhost:21651/EDI/Ecoporto/GetTableEcoporto/'+ _oArchive.id_TabelaEcoporto;

    window.open(url, "_parent");
    //window.top.jaddTab("_parent", url);
  };
  ...
  $scope.consultar = function()
  {
    ...
    var data = 
    {
      ...
      var url=  $scope.urlAPI+'EDI/Ecoporto/ImportacaoCliente';
      //var url=  'http://localhost:21651/EDI/Ecoporto/ImportacaoCliente';
        console.log(url);
        console.log(data);
      $http({
          method : "POST",
          ...
          $scope.responseEcoporto = response.data;
          console.log($scope.responseEcoporto);
          $scope.loadScreen = false;
      }, function myError(response) {
...
$scope.getRetorno = function()
{
  return $scope.responseEcoporto;
}
