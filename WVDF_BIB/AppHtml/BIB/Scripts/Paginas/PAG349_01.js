...
$('document').ready( function () {
	...
	$("#csag349__csag320_id").val(jCsag320Id);
	...
	$('#csag349__observacao').blur(function(){
		TextAreaLimite(this,1022);
	});
});
        
// SWITCH SECO
$(document).on('change', '#switchSeco', function() {
	$(this).prop("checked") ? switchSeco("S") : switchSeco("N");
});
function switchSeco(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchSeco').prop("checked", true) : $('#switchSeco').prop("checked", false);
		$('#csag349__armazem_seco').val(sTexto);
	} else {
		if ($('#csag349__armazem_seco').val() != "") {
			switchSeco($('#csag349__armazem_seco').val());
		} else {
			switchSeco($('#csag349__armazem_seco').val("N").val());
		}
	}
}

// SWITCH MOLHADO
$(document).on('change', '#switchMolhado', function() {
	$(this).prop("checked") ? switchMolhado("S") : switchMolhado("N");
});
function switchMolhado(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchMolhado').prop("checked", true) : $('#switchMolhado').prop("checked", false);
		$('#csag349__armazem_molhado').val(sTexto);
	} else {
		if ($('#csag349__armazem_molhado').val() != "") {
			switchMolhado($('#csag349__armazem_molhado').val());
		} else {
			switchMolhado($('#csag349__armazem_molhado').val("N").val());
		}
	}
}

// SWITCH CROSSDOCK
$(document).on('change', '#switchCrossdock', function() {
	$(this).prop("checked") ? switchCrossdock("S") : switchCrossdock("N");
});
function switchCrossdock(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchCrossdock').prop("checked", true) : $('#switchCrossdock').prop("checked", false);
		$('#csag349__crossdock').val(sTexto);
	} else {
		if ($('#csag349__crossdock').val() != "") {
			switchCrossdock($('#csag349__crossdock').val());
		} else {
			switchCrossdock($('#csag349__crossdock').val("N").val());
		}
	}
}

// SWITCH PROPRIA
$(document).on('change', '#switchPropria', function() {
	$(this).prop("checked") ? switchPropria("S") : switchPropria("N");
});
function switchPropria(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchPropria').prop("checked", true) : $('#switchPropria').prop("checked", false);
		$('#csag349__frota_propria').val(sTexto);
	} else {
		if ($('#csag349__frota_propria').val() != "") {
			switchPropria($('#csag349__frota_propria').val());
		} else {
			switchPropria($('#csag349__frota_propria').val("N").val());
		}
	}
}

// SWITCH TERCEIRIZADA
$(document).on('change', '#switchTerceirizada', function() {
	$(this).prop("checked") ? switchTerceirizada("S") : switchTerceirizada("N");
});
function switchTerceirizada(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchTerceirizada').prop("checked", true) : $('#switchTerceirizada').prop("checked", false);
		$('#csag349__frota_terceirizada').val(sTexto);
	} else {
		if ($('#csag349__frota_terceirizada').val() != "") {
			switchTerceirizada($('#csag349__frota_terceirizada').val());
		} else {
			switchTerceirizada($('#csag349__frota_terceirizada').val("N").val());
		}
	}
}

// SWITCH ESPECIAIS
$(document).on('change', '#switchEspeciais', function() {
	$(this).prop("checked") ? switchEspeciais("S") : switchEspeciais("N");
});
function switchEspeciais(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchEspeciais').prop("checked", true) : $('#switchEspeciais').prop("checked", false);
		$('#csag349__cargas_especiais').val(sTexto);
	} else {
		if ($('#csag349__cargas_especiais').val() != "") {
			switchEspeciais($('#csag349__cargas_especiais').val());
		} else {
			switchEspeciais($('#csag349__cargas_especiais').val("N").val());
		}
	}
}
