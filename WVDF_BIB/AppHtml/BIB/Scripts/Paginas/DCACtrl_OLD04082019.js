...
angular.module("CROSSApp").controller("DCACtrl", function($scope, DCAServicos, $filter){
    ...
    $scope.NewDCA = function()
    {
        /*
        $scope.oDCA = {};
        DCAServicos.getNewDca($scope.Id_BookingAtual).then(function(response){
            console.log("newDca", response);
    ...
    $scope.getDCA = function(_IdDCA)
    {
        console.log("DCA",_IdDCA)
        $scope.oDCA = {};
        DCAServicos.getDCA(_IdDCA).then(function(response){
    ...
    $scope.getDCAByBooking = function(_IdBooking)
    {
        DCAServicos.getByBooking(_IdBooking).then(function(response){
            console.log(response);
            $scope.lstDCA = response.data;
        }, function(err){
            console.log(err);
            parent.parent.alertify.error("Não foi possível carregar as UN's");
        });
    };
    ...
    load = function(){
        ...
        $scope.getDCAByBooking(getVariavelURL2("bookingId"));
    };
