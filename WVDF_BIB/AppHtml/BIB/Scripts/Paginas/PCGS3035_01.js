...
$(document).ready(function () {
	...
	$('#hcgs3035__vl_irrf').removeClass().addClass('col-xs-8');
	$('#hcgs3035__tp_custo_p').removeClass().addClass('form-control');
	$('#hcgs3035__vl_compra').removeClass().addClass('form-control');
	
	$('#switch-ID_FORMULA_P_checkbox').next().html('&nbsp;' + $('#id_formula_p').text());
	$('#id_formula_p').text('');
	
	$('#hcgs3035__formula_p').removeClass().addClass('form-control');
	
	$('#hcgs3035__tp_custo_r').removeClass().addClass('form-control');
	$('#hcgs3035__vl_venda').removeClass().addClass('form-control');
	$('#hcgs3035__tp_calculo_p').removeClass().addClass('form-control');
	$('#hcgs3035__tp_calculo_r').removeClass().addClass('form-control');
	$('#hcgs3035__vl_min').removeClass().addClass('form-control');
	...
	$('#hcgs3035__formula_r').removeClass().addClass('form-control');
	
	$('#switch-ID_DESCONTO_checkbox').next().html('&nbsp;' + $('#id_desconto').text());
	$('#id_desconto').text('');
	
	$('#hcgs3035__vl_aplicado').removeClass().addClass('form-control');
	
	$('input[type="checkbox"]').each(function(index, value) {
		var elementoFinal = ('hcgs3035__' + (value.id).replace('switch-', '').replace('_checkbox', '').toLowerCase());
	...
	$(document).on('change', '[id^="switch-"]', function() {
		var elementoFinal = 'hcgs3035__' + ($(this).attr('id').replace('switch-', '').replace('_checkbox', '').toLowerCase());
		configuraSwitchs($(this).prop('checked').toString(), $(this), elementoFinal);
	});

	if ($('#hcgs3035__moeda_p').val() != "") {
		var sParametros = $('#hcgs3035__moeda_p').val() + '","' + $('#jaUsuarioSessao').val();
		var sRetorno = ReadFuncValue(sParametros, "oPCGS218_01", "get_f_pgCCGS218_DESCRICAO");
			
		$('#pesquisaMoedaP').val(sRetorno);
	}

	if ($('#hcgs3035__moeda_r').val() != "") {
		var sParametros = $('#hcgs3035__moeda_r').val() + '","' + $('#jaUsuarioSessao').val();
		var sRetorno = ReadFuncValue(sParametros, "oPCGS218_01", "get_f_pgCCGS218_DESCRICAO");
	...
	aux = $('#hcgs3035__mod_pagmento_p').val();
	aux = aux.split(',');
	...
	aux = $('#hcgs3035__mod_pagmento_r').val();
	aux = aux.split(',');
	...
	$("#hcgs3035__tp_equipamento").autocomplete({
	...
	$("#pesquisaMoedaP").autocomplete({
        ...
        select: function(event, ui) {
            if (ui.item) {
				$('#hcgs3035__moeda_p').val(ui.item.id);
	...
	$("#pesquisaMoedaR").autocomplete({
        ...
        select: function(event, ui) {
            if (ui.item) {
				$('#hcgs3035__moeda_r').val(ui.item.id);
				$(this).val(ui.item.value);
				return false;
			}
        }
...
function jsSubmitForm(formulario) {
...
// ANGULAR JS
var app = angular.module('pcgs303501App', []);

app.controller('pcgs303501Controller', function($scope) {
	$scope.clickModPagamentoP = function() {
		...
		$('#hcgs3035__mod_pagmento_p').val(campoMP);
	};
	...
	$scope.clickModPagamentoR = function() {
		...
		$('#hcgs3035__mod_pagmento_r').val(campoMP);
	};
