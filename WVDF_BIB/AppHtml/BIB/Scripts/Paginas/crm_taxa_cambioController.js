var app = angular.module('taxaCambioApp', ['ngTable', 'ngRoute', 'ui.bootstrap', 'crmLibrary']);



app.controller('taxaCambioCoontroller', function ($scope, $http, NgTableParams, $location,$sce,crmLibrary) {
...
$scope.buscaTaxaCambio = function()
{
	$scope.loadScreen = true;
   crmLibrary.getRequest('crm/AcordosFinanceiros/TaxaCambio').then(function(response){
      	console.log(response);
        $scope.taxaCambio= response.data;
        $scope.loadScreen = false;
        $scope.populaTabelaTaxaCambio(response.data);
    }, function(response) {
       $scope.loadScreen = false;
       parent.parent.alertify.error("Ocorreu um erro ao buscar Taxas de Cambio.");
        console.log("Ocorreu um erro ao buscar Taxas de Cambio.");
    });
}
...
$scope.inserir = function()
{
   ...
   crmLibrary.postRequest('crm/AcordosFinanceiros/TaxaCambio/'+$scope.sessaoUsuario, data).then(function(response){
      	console.log(response);
        $scope.responseAdicionarPagamento = response.data;
        $scope.buscaTaxaCambio();
        $scope.populaTabelaTaxaCambio();
         $scope.loadScreen = false;
...
$scope.alterar = function()
{
	...
	var data = {
	"id_ref_taxa_cambio": $scope.selected.id_ref_taxa_cambio,
	"no_referencia": $scope.editNome, 
	"ds_referencia": $scope.editDescricao};
	console.log('alterar - data', data);
    	  $scope.loadScreen = true;

   crmLibrary.putRequest('crm/AcordosFinanceiros/TaxaCambio/'+$scope.sessaoUsuario, data).then(function(response){
      	console.log(response);
        $scope.responseAdicionarPagamento = response.data;
        $scope.buscaTaxaCambio();
        $scope.populaTabelaTaxaCambio();
...
$scope.remover = function()
{
    	  ...
    	  $scope.loadScreen = true;
   crmLibrary.deleteRequest('crm/AcordosFinanceiros/TaxaCambio/'+$scope.selected.id_ref_taxa_cambio+'/'+$scope.sessaoUsuario).then(function(response){
      	console.log(response);
        $scope.buscaTaxaCambio();
        $scope.populaTabelaTaxaCambio();
...
console.log('sessao',$scope.sessaoUsuario);
 console.log($scope.buscaTaxaCambio());
...
$scope.populaTabelaTaxaCambio = function(data)
{
		$scope.tableTaxaCambio = new NgTableParams({
			page: 1,
			count: 10,
