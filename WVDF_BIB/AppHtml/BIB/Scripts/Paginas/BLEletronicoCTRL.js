...
angular.module("CrossApp").controller("blEletronicoCtrl", function($scope, BLServicos, $location, $window){
    ...
    {   
        ...
        if($scope.id_BLEletonico > 0) 
        {
            $scope.loadingState = true;
            BLServicos.getById($scope.id_BLEletonico, $scope.ds_Token).then(function(response){
        ...
        {
            $scope.loadingState = true;
            if(getVariavelURL('tipoAcesso') == 1)
            {
                BLServicos.getNewBLClient($scope.id_Booking, $scope.ds_Token).then(function(response){
                    $scope.oBL = response.data;
                    $scope.loadingState = false;
            ...
            {
                BLServicos.getNewBL($scope.id_Booking, $scope.ds_Token).then(function(response){
                    $scope.oBL = response.data;
                    $scope.loadingState = false;
                },function(err){
                    parent.parent.alertify.error(err);
