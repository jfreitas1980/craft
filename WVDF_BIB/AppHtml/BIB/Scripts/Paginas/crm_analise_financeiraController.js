var analiseNeutralidadae = angular.module("analiseFinanceira", ["ngTable", "ngRoute", "ui.bootstrap"]);



analiseNeutralidadae.controller("analiseFinanceiraController", function Ctrl($scope, $filter, NgTableParams, $http, $window, $location, $sce) {
 ...
 //  }
  ...
  //$scope.serasaURL = "\crm_serasa_relatoMais.html?&documento="+$scope.selected.doc_ref+"?aUsuarioSessao="+$scope.sessao;    
  ...
  $scope.getUrlSerasa = function()
  {

   $scope.serasaURL = "\crm_serasa_relatoMais.html?documento="+$scope.selected.doc_ref+"&aUsuarioSessao="+$scope.sessao+"&busca=true";    
   console.log('urlserasa', $scope.serasaURL);
      // alert($scope.serasaURL);
       //alert('teste',$scope.serasaUR );
       return $scope.serasaURL;
     }
...
$scope.aprovaFinanceiro =function()
{
 ...
 var selected = $scope.selected;
   ...
   console.log($scope.selected);
	   ...
	   if(!$scope.validadeCadastro || $scope.validadeCadastro < 0 && $scope.validadeCadastro > 120)
	  {
		  parent.parent.alertify.error('Validade do cadastro deve estar entre 1 e 120 dias.');
        return;
	  }
      ...
      //var urlReq = 'http://localhost:21651/crm/pendencias/2/'+$scope.sessao;
      var urlReq = _getUrlAPI() + '/crm/pendencias/2/'+$scope.sessao;
    ...
    .then(function(response) {
      ...
      $scope.obsFinan = '';
      $scope.validadeCadastro = '';
      parent.parent.alertify.success('Analise Aprovada!'); 
  ...
  console.log($scope.validadeCadastro);
