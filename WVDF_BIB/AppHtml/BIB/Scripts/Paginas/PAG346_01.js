...
$('document').ready( function () {
	...
	$("#csag346__csag320_id").val(jCsag320Id);
	...
	$('#csag346__observacao').blur(function(){
		TextAreaLimite(this,1022);
	});
});

// SWITCH MAPA
$(document).on('change', '#switchMapa', function() {
	$(this).prop("checked") ? switchMapa("S") : switchMapa("N");
});
function switchMapa(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchMapa').prop("checked", true) : $('#switchMapa').prop("checked", false);
		$('#csag346__gera_mapa').val(sTexto);
	} else {
		if ($('#csag346__gera_mapa').val() != "") {
			switchMapa($('#csag346__gera_mapa').val());
		} else {
			switchMapa($('#csag346__gera_mapa').val("N").val());
		}
	}
}

// SWITCH XML
$(document).on('change', '#switchXml', function() {
	$(this).prop("checked") ? switchXml("S") : switchXml("N");
});
function switchXml(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchXml').prop("checked", true) : $('#switchXml').prop("checked", false);
		$('#csag346__gera_xml').val(sTexto);
	} else {
		if ($('#csag346__gera_xml').val() != "") {
			switchXml($('#csag346__gera_xml').val());
		} else {
			switchXml($('#csag346__gera_xml').val("N").val());
		}
	}
}

// SWITCH SIMULADOR
$(document).on('change', '#switchSimulator', function() {
	$(this).prop("checked") ? switchSimulator("S") : switchSimulator("N");
});
function switchSimulator(sTexto) {
	if (sTexto != "") {
		(sTexto == "S") ? $('#switchSimulator').prop("checked", true) : $('#switchSimulator').prop("checked", false);
		$('#csag346__armazenagem_simulator').val(sTexto);
	} else {
		if ($('#csag346__armazenagem_simulator').val() != "") {
			switchSimulator($('#csag346__armazenagem_simulator').val());
		} else {
			switchSimulator($('#csag346__armazenagem_simulator').val("N").val());
		}
	}
}
