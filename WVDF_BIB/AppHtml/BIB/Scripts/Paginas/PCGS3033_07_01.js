...
$(document).ready(function() {
	...
	$(document).on('click', '#tb1', function() {
		sParametros = '","' + getVariavelURL('sProtocolo') + '","2';
		console.log(sParametros);
		sRetorno = ReadFuncValue(sParametros, "oPCGS3033_01", "Get_f_PegaTitulo");
		$('#titulo').html(sRetorno);
	});
...
app.controller('pcgs303307Controller', function($scope, buscaWS, $http, $filter, $location) {
	...
	$scope.limparTela = function() {
		...
		acordo.camposWS.sCSAG308_ID = ($scope.marca == undefined) ? "" : $scope.marca
  ...
  $scope.$watch('dtEnd', function(novo) {
	...
	$scope.gerarAcordo = function() {
		...
		$http({
			...
			if ((data.data.MSG).substr(0, 4) == "ERR#") {
				parent.alertify.error(data.data.MSG.substr(4));
				return;
			}
	...
	$scope.posicionaAcordo = function() {
		buscaWS.get('/WVDF_WS/ws_hcgs3033.wso/f_busca_acd/JSON', 'sProt=' + $scope.idProtocolo).then(function(data) {
			...
			// Monta Select
			buscaWS.get('/WVDF_WS/ws_hcgs3034.wso/f_PesquisaHCGS3034/JSON', 'aHCGS3033_ID=' + acordo.camposWS.sHCGS3033_ID).then(function(data) {
				$scope.taxasAcordadas = data;
			});
		});
	}; 
	...
	$scope.clickTaxasAcordadas = function(row35, row36, row37, id34) {
		src = "PCGS3035_01.asp?aUsuarioSessao=" + getVariavelURL('aUsuarioSessao') + "&aHCGS3034Id=" + id34 + "&aHCGS3033Id="+ acordo.camposWS.sHCGS3033_ID +"&RowId=" + row35;
		$('#iframe-pcgs3035_01').attr('src', src);

		src = "PCGS3036_01.asp?aUsuarioSessao=" + getVariavelURL('aUsuarioSessao') + "&aHCGS3034Id=" + id34 + "&aHCGS3033Id="+ acordo.camposWS.sHCGS3033_ID +"&RowId=" + row36;
		$('#iframe-pcgs3036_01').attr('src', src);

		src = "PCGS3037_01.asp?aUsuarioSessao=" + getVariavelURL('aUsuarioSessao') + "&aHCGS3034Id=" + id34 + "&aHCGS3033Id="+ acordo.camposWS.sHCGS3033_ID +"&RowId=" + row37;
		$('#iframe-pcgs3037_01').attr('src', src);

		buscaWS.get('/WVDF_WS/ws_hcgs3033.wso/f_prod_hcgs3033/JSON', 'sHCGS3033_ID=' + acordo.camposWS.sHCGS3033_ID).then(function(data) {
			$scope.listaOperacoes = data.split(',');
			console.log($scope.listaOperacoes[0])
			console.log(data.split(','));
		});

		$scope.taxaSelecionada = true;
		setTimeout(function() { $('#tb2').click(); }, 500 );
		
   		sRetorno = ReadFuncValue(id34, "oPCGS3033_01", "Get_f_PegaTituloTaxa");
   		$('#titulo').html(sRetorno);
