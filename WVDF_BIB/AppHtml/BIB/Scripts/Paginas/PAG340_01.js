...
$(document).ready( function () {
	...
	var aCSAG320_CODIGO = $('#csag340__csag320_id').val();
	...
	$("#modal-grp_clientes").attr({
		...
		"href":"phsag340_04.asp?SelStart1=" + aCSAG340_CODIGO + "&SelStart2=" + aCSAG320_CODIGO + "&Index=1&RunReport=RunReport&aUsuarioSessao=" + vUsuarioSessao
	});

	jCsag320Id = $('#jCsag320Id').val();
	$("#csag340__csag320_id").val(jCsag320Id);
	
	$('#csag340__situacao').removeClass('col-xs-6').addClass('col-xs-2');
	
	$('#csag340__situacao_dthr').removeClass('col-xs-6').addClass('col-xs-4');
	$('#csag340__situacao_dthr').height(15);
	...
	$("#modal-grp_clientes").fancybox({
		...
		autoSize	: false,
		modal		: false,
		afterClose	:  function () { $('#btnSalvar').click(); $('#popup_ok').click(); },
		helpers: {
			overlay: {
				locked: false
			}
	...
	$('#csag340__observacao').blur(function(){
