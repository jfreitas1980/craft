$(document).ready(function () {
	var listas = $('#listaDupla_esquerda').bootstrapDualListbox({
		nonSelectedListLabel: '<b>INCOTERM</b>',
		selectedListLabel: '<b>Operacao x Incoterm</b>',
		infoTextEmpty: '',
		infoText: false,
	...
	$(document).on('click', '#btnPesquisar', function() {
		if ($('#listaUsuarios').val() != "0") {
			...
			$.ajax({
				url: '/WVDF_WS/ws_ccgs237.wso/f_PesquisaCCGS237/JSON',
				type: 'POST',
				 data: { 
					'aUsuarioSessao': getVariavelURL('aUsuarioSessao'), 
					'aCCGS200_ID': $('#listaOperacao').val()
				},
				dataType: 'json',
	...
	$(document).on('change', '#listaDupla_esquerda', function() {
		...
		if (valoresMarcados != null) {
			$.ajax({
				type: 'POST',
				url: '/WVDF_WS/ws_ccgs237.wso/f_AtualizaCCGS237/JSON',
				data: { 
					'aUsuarioSessao': getVariavelURL('aUsuarioSessao'), 
					'aCCGS200_ID': $('#listaOperacao').val(),
