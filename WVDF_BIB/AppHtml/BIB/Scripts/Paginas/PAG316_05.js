...
app.controller('pag31605Ctrl', ['$scope', 'callWS', '$timeout', function($scope, callWS, $timeout) {
    ...
    var getDualList = function(currentPage, aOption, aPessoa) {
        ...
        callWS.get('/WVDF_WS/ws_csag316.wso/f_PesquisaCSAG316/JSON', params).then(function(response) {
    ...
    var atualizaWS = function(value) {
        var params = {
            'aUsuarioSessao': aUsuarioSessao,
            'aCSAG300_ID':  $scope.usuario.id,
            'aCSAG308_ID': value.ID,
            'bSel': value.SEL
        };
      //  console.log(params);
        callWS.get('/WVDF_WS/ws_csag316.wso/f_AtualizaCSAG316/JSON', params).then(function(response) {});
    }
