...
$scope.busca = function()
{
   ...
   crmLibrary.getRequest('crm/GrupoEconomico/Pessoas/'+$scope.documento).then(function(response){
      	console.log(response);
        $scope.grupoEconomicoResponse= response.data;
        $scope.loadScreen = false;
        $scope.populaTabelaTaxaMercado(response.data.grupo);
    }, function(response) {
       $scope.loadScreen = false;
...
$scope.populaTabelaTaxaMercado= function(data)
{
		$scope.tablelaTaxaMercado = new NgTableParams({
			page: 1,
			count: 10,
      sorting: { no_documento_formatado: 'asc' },
		}, {
			counts: [10],
			dataset: data
		});
