var app = angular.module('pag40005', ['ui.bootstrap', 'wsDominio', 'diretivas']);

app.controller('pag40005Ctrl', ['$scope', 'callWS', '$timeout', function($scope, callWS, $timeout) {
    ...
    var getDualList = function(currentPage, aOption, aPessoa) {
        ...
        callWS.get('/WVDF_WS/ws_csag400.wso/f_PesquisaCSAG400/JSON', params).then(function(response) {
    ...
    var atualizaWS = function(value) {
        var params = {
            'aUsuarioSessao': aUsuarioSessao,
            'aGRUPO_ID':  $scope.grupo.id,
            'aSISTEMA_ID': value.ID,
            'bSel': value.SEL
        };
      //  console.log(params);
        callWS.get('/WVDF_WS/ws_csag400.wso/f_AtualizaCSAG400/JSON', params).then(function(response) {});
    }
