// ANGULAR JS
app = angular.module('pcgs210105NewApp', ['ngSanitize', 'Alertify', 'angularSoap', 'diretivas', 'wsDominio', 'smart-table', 'ui.bootstrap', 'datatables', 'datatables.buttons', 'datatables.colreorder', 'ngTagsInput', 'toaster']);
...
app.controller('pcgs210105NewController', function($scope, Alertify, buscaWS, callWS, $http, $q, $compile, toaster, $uibModal) {
...
*/
    ...
    $scope.acCidades_D = function(query) {
        var paisID = $scope.fPaisD;
        if (paisID == undefined)
            paisID = '';

        var data = buscaWS.get('/WVDF_WS/ws_csag325.wso/buscarCidadesPorPaisesPorTrade/JSON', 'aUsuarioSessao=' + getVariavelURL('aUsuarioSessao') + '&idTrade=&idPais=' + paisID + '&sCidade=' + query)
            .then(function(data) {
                // console.log(data);
                return data;
            });
   ...
   // buscaWS.get('/WVDF_WS/ws_csag379.wso/f_tradeline_list/JSON', '').then(function(data) {
    ...
    var getParamsPesquisa = function() {
        ...
        parametros.sDes = auxPod;
        parametros.sPais = ($scope.fPais == undefined) ? "" : $scope.fPais;
        parametros.sPaisD = ($scope.fPaisD == undefined) ? "" : $scope.fPaisD;
        parametros.sDtBgn = ($scope.mDataInicial == undefined) ? "" : $scope.mDataInicial;
        parametros.sDtEnd = ($scope.mDataFinal == undefined) ? "" : $scope.mDataFinal;
        parametros.sContrato = ($scope.fContrato == undefined) ? "" : $scope.fContrato;
        parametros.sDescricao = ($scope.fDescricao == undefined) ? "" : $scope.fDescricao;
