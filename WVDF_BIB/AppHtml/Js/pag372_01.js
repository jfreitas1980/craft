                ...
                $('document').ready( function () {
                    $('#csag372__csag308_nr').removeClass('ddEntryCombo');
                    $('#csag372__tipo').removeClass('ddEntryCombo');
                    
                    $('#csag372__csag320_nm').addClass('autocompletecsag320_nm');   
                    $(".autocompletecsag320_nm").autocomplete({
                        source: function( request, response ) {
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    aCSAG308_NR: $('#csag372__csag308_nr option:selected').val(),
                                    term: request.term
                                },
                                dataType: "json",
                        ...
                        select: function( event, ui ) {
                            if (ui.item) {
                                ...
                                $("#csag372__csag320_cd").val(selecionado_id);
                    ...
                    //blur
                    $('#csag372__csag320_cd').blur(function(){
                        var acsag308_nr = $('#csag372__csag308_nr option:selected').val();
                        var sRetorno = ReadFuncValue($(this).val()+'","'+acsag308_nr,"oPAG320_01","get_f_pegaDescricao");
                        $('#csag372__csag320_nm').val(sRetorno);
                    });

                    $('#csag372__familiares').blur(function(){
                        limite(this,1006);
                    });
                    
                    
                    // foi implementado para que a classe do autocomplete seja refeita com a empresa selecionada.
                    $('#csag372__csag308_nr').change(function(){
                        $(".autocompletecsag320_nm").autocomplete({
                            source: function( request, response ) {
                                $.ajax({
                                    url: "/wvdf_cac_8/fbcsag320_clientenome.asp",
                    ...
                    $('#btn-SAVE').click(function(){
                        var aEmpresa = $('#csag372__csag308_nr').val();
                        if (aEmpresa == 0 || aEmpresa == '') {
                          jAlert('warning', 'Selecione uma Empresa', 'Atencao', function(r){
                              if (r) {
                                $('#csag372__csag308_nr').select();
                                $('#csag372__csag308_nr').focus();
                                return false;
                              }
                          });
                        } else { jsOK=0 };
