...
$(document).ready(function() {
	...
	$("#fileUpload3").fileUpload({
		...
		'onError': function (a, b, c, d) {
         if (d.status == 404)
            alert('1: Could not find upload script. Use a path relative to: '+'<?= getcwd() ?>');
         else if (d.type === "HTTP")
            alert('2: error '+d.type+": "+d.status);
         else if (d.type ==="File Size")
            alert('3:' + c.name+' '+d.type+' Limit: '+Math.round(d.sizeLimit/1024)+'KB');
         else
            alert('4: error '+d.type+": "+d.text);
}
	});
});
